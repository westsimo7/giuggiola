...
function App() {
  const { ARTICLES, BOOKS } = window.SITE_DATA;
  const [t, setTweak] = useTweaks(TWEAK_DEFAULTS);
  const [view, setView] = useState({ name: "home" });
  const [library, setLibrary] = useState([]); // purchased book ids
  ...
  const go = useCallback((name) => {
    ...
    setView({ name });
    if (name !== "home") window.scrollTo({ top: 0, behavior: "instant" in window ? "instant" : "auto" });
  }, []);
