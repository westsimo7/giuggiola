// components.jsx — UI building blocks for Giuggiola's Think
...
function ArrowLink({ children, onClick, href }) {
  const props = href ? { href } : { onClick, role: "button", tabIndex: 0 };
  return (
    <a className="arrow-link" {...props}>
      {children}
...
function Navbar({ go, current, cartCount, onCart }) {
...
function SectionHead({ kicker, title, intro, action }) {
  return (
...
function ArticleCard({ article, onOpen, index }) {
  return (
...
/* ---------- Book cover (typographic, no faux imagery) ---------- */

function BookCover({ book, small }) {
  return (
...
function BookCard({ book, onBuy, index }) {
  return (
...
function ArticleView({ article, go, related, onOpen }) {
...
function PurchaseModal({ book, onClose, onConfirm }) {
...
Object.assign(window, {
  Kicker, ArrowLink, Navbar, Hero, SectionHead, ArticleCard, BookCover,
  BookCard, ChiSono, Footer, ArticleView, PurchaseModal,
});
